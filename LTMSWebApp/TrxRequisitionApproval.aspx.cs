...
{
    public partial class TrxRequisitionApproval : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Status = "Rejected";
                    objRequisition.SaveStatus = 3; 
                }                             
 
                objRequisition.UserId = ((ClsUserInfo)Session["UserInfo"]).UserId; 
                objRequisition.IpAdd = Request.UserHostAddress;
                bool IsUpdated = objLtmsService.UpdateApprovalInRequisition(objRequisition,ReqId);
                if (IsUpdated == true)
                {
        ...
        protected void GvData_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                try
                {
                    ...
                    if (StatusVal == 1) { Status = "<font color='red'>Approval Pending</font>"; }
                    else if (StatusVal == 2) { Status = "First Leval Approved"; }
                    else if (StatusVal == 3) { Status = "First Leval Reject"; }
                    else if (StatusVal > 3) { Status = "First Leval Approved"; }                 
