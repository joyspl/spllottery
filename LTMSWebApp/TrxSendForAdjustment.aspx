<%@ Page Title="" Language="C#" MasterPageFile="~/WebPageMaster.Master" AutoEventWireup="true" CodeBehind="TrxSendForAdjustment.aspx.cs" Inherits="LTMSWebApp.TrxSendForAdjustment" %>
<%@ Register Assembly="AjaxControlToolkit" Namespace="AjaxControlToolkit" TagPrefix="aspctr" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type='text/javascript' language="javascript">
        function checkAllBoxes() {
            var totalChkBoxes = parseInt('<%= this.GvTransaction.Rows.Count %>');
            var gvControl = document.getElementById('<%= this.GvTransaction.ClientID %>');
        ...
        function CalculateAdjustment() {         
            ...
            var GridviewRows = $("#<%=GvTransaction.ClientID%> tr").length;                
            var TotalSale = document.getElementById('AppContentPlaceHolder_txtTotalSale').value;
            var Amount = 0;
            var rowlenght = GridviewRows - 1;
            for (var i = 0; i < rowlenght; i++) {
                if (document.getElementById("AppContentPlaceHolder_GvTransaction_chkSelect_" + [i] + "").checked) {
                    Amount =Amount+ (-1 *  parseFloat(document.getElementById("AppContentPlaceHolder_GvTransaction_lblAmount_" + [i] + "").innerHTML));                    
                }
            }
           
            var AdjAmount = parseFloat(document.getElementById("AppContentPlaceHolder_hdAdjustmentAmount").value);
            Amount = parseFloat(parseFloat(Amount) + parseFloat(AdjAmount));
            document.getElementById("AppContentPlaceHolder_txtSentForAdusutment").value = parseFloat(Amount);
            document.getElementById("AppContentPlaceHolder_txtRemainingBalence").value = parseFloat(TotalSale - Amount);
            document.getElementById("AppContentPlaceHolder_txtRemainingBalence").style.backgroundColor = "#6699ff";
            if (parseFloat(document.getElementById("AppContentPlaceHolder_txtRemainingBalence").value) < 0) {
                document.getElementById("AppContentPlaceHolder_txtRemainingBalence").style.backgroundColor = "#ff3300";
            }
        ...
        function isValidEntry() {
            if (parseFloat(document.getElementById("AppContentPlaceHolder_txtRemainingBalence").value) < 0){
                alert("Remaining balance can not be more than the sale ticket amount");                
                return false;
            }
            return true;
        }
    ...
    <div class="container-fluid">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">
                <asp:Label ID="lblHeader" runat="server" Text="Send for Adjustment"></asp:Label>
            </li>
        </ol>
        <div class="row">
            <div class="col-12">
                               ...
                               HeaderStyle-Font-Size="12px" RowStyle-Font-Size="10px" >
                                <Columns>
                                    <asp:TemplateField>
                                        ...
                                        <ItemTemplate>                                 
                                            <asp:CheckBox ID="chkSelect" onClick="CalculateAdjustment();"  runat="server" /></ItemTemplate>
                                    </asp:TemplateField>
                        ...
                        <div style="width:900px; margin:10px auto;">
                            <table>
                                <tr>
                                    ...
                                    <td colspan="3">
                                        <asp:Button ID="btnConfirm" runat="server" Text="Send for Adjustment"  onClientClick="return isValidEntry();"  CssClass="btn btn-primary" CommandName="View" OnClick="btnSave_Click" />
